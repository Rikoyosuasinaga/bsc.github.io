...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('.dataTable').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('admin/slider/listdata') ?>",
                "type": "POST"
            },
    ...
    function add()
    {
        ...
        $('.modal-title').text('Tambah Banner Slider'); // Set Title to Bootstrap modal title
        $('#btnSave').text('Simpan');
        $('#photo-preview').hide(); // hide photo preview modal
        $('#label-photo').text('Upload Banner Slider'); // label photo upload
    }
    ...
    function editdata(id)
    {
        ...
        $.ajax({
            url: "<?php echo site_url('admin/slider/edit') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                ...
                $('#tambah').modal('show'); // show bootstrap modal
                $('.modal-title').text('Edit Banner Slider'); // Set Title to Bootstrap modal title
                $('#btnSave').text('Update');
                $('#photo-preview').show(); // hide photo preview modal
                if (data.banner_slider)
                {
                    $('#label-photo').text('Change Banner Slider'); // label photo upload
                    $('#photo-preview div').html('<img src="' + base_url + 'slider/' + data.banner_slider + '" class="img-fluid">'); // show photo
                } else
                {
                    $('#label-photo').text('Upload Banner Slider'); // label photo upload
    ...
    function save()
    {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('admin/slider/save') ?>";
        } else {
    ...
    function deletedata(id)
    {
            ...
            $.ajax({
                url: "<?php echo site_url('admin/slider/hapus') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
                        ...
                        <div class="col-sm-12">
                            <div class="material-group material-group-danger">
                                ...
                                <div class="form-group form-danger form-static-label">
                                    <input class="form-control" id="fileup" style="display: none" name="banner_slider" type="file" onchange="document.getElementById('namafile').value=this.value">
