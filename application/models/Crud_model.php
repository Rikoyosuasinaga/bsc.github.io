...
class Crud_model extends CI_Model {
    ...
    function readlimit($table, $where = null, $order = null, $limit = 0) {
        if ($where == null && $order == null) {
            return $this->db->get($table, $limit)->result_array();
        } else if ($where != null && $order == null) {
            return $this->db->where($where)->get($table, $limit)->result_array();
        } else if ($where == null && $order != null) {
            return $this->db->order_by($order)->get($table, $limit)->result_array();
        } else {
            return $this->db->where($where)->order_by($order)->get($table, $limit)->result_array();
        }
    }
